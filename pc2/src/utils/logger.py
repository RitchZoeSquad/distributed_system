...
class Logger:
    def __init__(self, name: str):
        ...
        json_handler = RotatingFileHandler(
            filename=f"/app/logs/pc2_{datetime.now().strftime('%Y%m%d')}.json",
        ...
        text_handler = RotatingFileHandler(
            filename=f"/app/logs/pc2_{datetime.now().strftime('%Y%m%d')}.log",
            maxBytes=10*1024*1024,  # 10MB
            backupCount=5
        )
        ...
        error_handler = RotatingFileHandler(
            filename=f"/app/logs/pc2_errors_{datetime.now().strftime('%Y%m%d')}.log",
