...
class Logger:
    def __init__(self, name: str):
        ...
        json_formatter = jsonlogger.JsonFormatter(
            fmt='%(asctime)s %(name)s %(levelname)s %(message)s %(pc_id)s %(service_type)s',
            datefmt='%Y-%m-%d %H:%M:%S'
        )
        ...
        json_handler = RotatingFileHandler(
            filename=f"/app/logs/pc4_{datetime.now().strftime('%Y%m%d')}.json",
        ...
        text_handler = RotatingFileHandler(
            filename=f"/app/logs/pc4_{datetime.now().strftime('%Y%m%d')}.log",
            maxBytes=10*1024*1024,  # 10MB
            backupCount=5
        )
        ...
        for service in ['domain_email', 'phone']:
            error_handler = RotatingFileHandler(
                filename=f"/app/logs/pc4_{service}_errors_{datetime.now().strftime('%Y%m%d')}.log",
    ...
    def error(self, message: str, service_type: str = None, exc_info: bool = True, extra: dict = None):
    ...
    def warning(self, message: str, service_type: str = None, extra: dict = None):
        """Log warning message"""
        if service_type:
            extra = extra or {}
            extra['service_type'] = service_type
    ...
    def critical(self, message: str, service_type: str = None, extra: dict = None):
    ...
    def exception(self, message: str, service_type: str = None, extra: dict = None):
